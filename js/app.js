...
let cropper = null;
let mgrEditingItemId;
let mgrEditingCategoryId;
let pendingItemImageBlob = null;
let pendingItemImagePreviewUrl = null;
let pendingCategoryImageBlob = null;
let pendingCategoryImagePreviewUrl = null;
const money = (value) => `${Number(value).toFixed(0)} EGP`;
let categories = [];
const orderStatuses = [
  "AWAITING_PAYMENT",
  "APPROVED",
...
const initialTables = [
  {
    ...
    token: "pm-t01-7c2",
    area: "Terrace",
    sessions: [{ id: "S-101", total: 520, closedAt: "Mon" }],
  },
  {
    id: "02",
    label: "Table 02",
    ...
    area: "Window",
    sessions: [
      { id: "S-102", total: 320, closedAt: "Tue" },
      { id: "S-118", total: 415, closedAt: "Wed" },
    ],
  },
  {
    id: "03",
    ...
    token: "pm-t03-4e8",
    area: "Main hall",
    sessions: [{ id: "S-103", total: 680, closedAt: "Tue" }],
  },
  {
    id: "04",
    label: "Table 04",
    ...
    token: "pm-t04-2a4",
    area: "Main hall",
    sessions: [{ id: "S-104", total: 290, closedAt: "Wed" }],
  },
  {
    id: "05",
    label: "Table 05",
    ...
    token: "pm-t05-5f6",
    area: "Garden",
    sessions: [{ id: "S-105", total: 560, closedAt: "Thu" }],
  },
  {
    id: "06",
    label: "Table 06",
    ...
    token: "pm-t06-8b0",
    area: "Window",
    sessions: [{ id: "S-106", total: 210, closedAt: "Thu" }],
  },
  {
    id: "07",
    label: "Table 07",
    ...
    guests: 2,
    status: "OCCUPIED",
    token: "cbbf9ba1-283d-4eb0-8ccd-a292a83026c3",
    area: "Terrace",
    sessions: [
      { id: "S-107", total: 520, closedAt: "Mon" },
      { id: "S-111", total: 780, closedAt: "Tue" },
      { id: "S-115", total: 420, closedAt: "Wed" },
    ],
  },
  {
    id: "08",
    ...
    token: "pm-t08-3d7",
    area: "Garden",
    sessions: [{ id: "S-108", total: 610, closedAt: "Thu" }],
  },
  {
    id: "09",
    label: "Table 09",
    ...
    token: "pm-t09-6f1",
    area: "Bar",
    sessions: [{ id: "S-109", total: 185, closedAt: "Fri" }],
  },
  {
    id: "10",
    label: "Table 10",
    ...
    token: "pm-t10-8a9",
    area: "Private room",
    sessions: [{ id: "S-110", total: 1240, closedAt: "Fri" }],
  },
];
const seededOrders = [
  {
  ...
  },
  {
    ...
    items: [
      {
        id: "seed-1041",
        menuItemId: "cheese-burger",
        name: "Cheese Burger",
        image: "🍔",
        unitPrice: 295,
        quantity: 2,
        options: ["Double", "Extra cheese"],
        notes: "",
      },
      {
        id: "seed-1041b",
        menuItemId: "classic-fries",
        name: "Classic Fries",
        image: "🍟",
        unitPrice: 95,
        quantity: 1,
  ...
  },
  {
    ...
    customRequest: "Add a birthday message if possible",
    status: "APPROVED",
    paymentStatus: "PAID",
    paymentMethod: "Card",
    paidAmount: 310,
...
const defaultState = {
  ...
  notices: [
    {
      id: "welcome",
      title: "Welcome to PM",
      body: "Scan your table to start an order.",
      tone: "info",
      audience: "CUSTOMER",
      createdAt: "Now",
    },
  ...
  notes: [
    {
      id: "note-1",
      text: "Table 07 requested extra napkins.",
      author: "Floor team",
      audience: "Manager/Admin",
      createdAt: "Today, 6:02 PM",
    },
  ],
  staff: [],
};
let state = loadState();
let customerView = state.customerTableId ? "menu" : "menu";
const CUSTOMER_CATEGORY_SCROLL_KEY = "pm-cafe-category-scroll-left";
const CUSTOMER_CATEGORY_KEY = "pm-cafe-selected-category";
let selectedCategory = safeGetStorage(CUSTOMER_CATEGORY_KEY) || "Pizza";
let activeProduct = null;
let staffSection = "overview";
let cashierFilter = "ALL";
let kitchenSearch = "";
function apiBase() {
  return String(window.PM_API_URL || "http://127.0.0.1:8000").replace(
    /\/$/,
    "",
  );
}
...
function clearAuth() {
  authSession = null;
  try {
    localStorage.removeItem("pm-cafe-auth");
  } catch {
    /* storage unavailable */
  }
}
...
async function authFetch(path, options = {}, retry = true) {
  ...
  if (authSession?.access)
    headers.set("Authorization", `Bearer ${authSession.access}`);
  if (
    options.body &&
...
async function loginWithBackend(username, password, expectedRole) {
  let tokenResponse;
  try {
  ...
  if (!meResponse.ok) {
    clearAuth();
    return "This account has no staff profile.";
  }
...
let mgrTables = null;
let mgrStaff = null;
let mgrAttendance = null;
let mgrAttendanceDate = new Date().toISOString().slice(0, 10);
let mgrCategories = null;
let mgrMenuItems = null;
let mgrAnalytics = null;
let mgrError = "";
...
function resetManagerCaches() {
  ...
  mgrMenuFilter = "ALL";
  mgrMenuSearch = "";
  mgrMenuLoading = false;
  mgrAnalyticsLoading = false;
  mgrAnalyticsTried = false;
  mgrAnalyticsError = "";
}
...
async function loadManagerTables() {
  try {
    mgrTables = await apiJson("/api/tables/");
  } catch (error) {
    mgrError =
...
async function loadManagerStaff() {
  try {
    mgrStaff = await apiJson("/api/staff/");
  } catch (error) {
    mgrError =
...
async function loadManagerAttendance() {
  try {
    ...
    mgrAttendance = await apiJson(`/api/attendance/?date=${mgrAttendanceDate}`);
  } catch (error) {
    mgrError =
...
async function loadManagerCategories() {
  try {
    mgrCategories = await apiJson("/api/categories/");
  } catch (error) {
    mgrError =
...
async function loadManagerMenuItems() {
  try {
    mgrMenuItems = await apiJson("/api/menu/");
  } catch (error) {
    mgrError =
...
async function refreshManagerMenu() {
  mgrMenuLoading = true;
  try {
    await Promise.all([loadManagerCategories(), loadManagerMenuItems()]);
  } finally {
    mgrMenuLoading = false;
  }
  hydrateMenu().catch(() => {});
...
async function loadManagerAnalytics() {
  ...
  mgrAnalyticsError = "";
  try {
    mgrAnalytics = await apiJson("/api/analytics/");
  } catch (error) {
    mgrAnalyticsError =
...
async function loadKitchenOrders() {
  try {
    kitchenOrders = await apiJson("/api/orders/?ordering=priority,created_at");
    kitchenError = "";
  } catch (error) {
    kitchenError =
      error instanceof Error
        ? error.message
        : "Could not load the kitchen queue.";
  }
  kitchenLoaded = true;
  render();
}
...
function stopKitchenPolling() {
  if (kitchenPollTimer !== null) {
    window.clearInterval(kitchenPollTimer);
...
let stfError = "";
let stfPollTimer = null;
async function loadStaffOrders() {
  try {
    stfOrders = await apiJson("/api/orders/?ordering=priority,created_at");
    stfError = "";
  } catch (error) {
    stfError =
...
function stopStaffPolling() {
  if (stfPollTimer !== null) {
    window.clearInterval(stfPollTimer);
    stfPollTimer = null;
  }
}
...
async function loadStaffTables() {
  try {
    stfTables = await apiJson("/api/tables/");
  } catch (error) {
    stfError =
      error instanceof Error ? error.message : "Could not load tables.";
    stfTables = [];
...
async function loadReservations() {
  try {
    cashReservations = await apiJson("/api/reservations/");
    reservationError = "";
  } catch (error) {
    reservationError =
...
function reservationTablesAvailableFor(
  dateValue,
  startTime,
  durationMinutes,
  ...
  cashReservations
    ...
    .forEach((r) => {
      ...
      if (start < rEnd && rStart < end)
        r.tables.forEach((id) => blockedTableIds.add(id));
    });
  return tables.filter((t) => t.active && !blockedTableIds.has(t.id));
}
function reservationFormValues() {
  const customerName = document.getElementById("res-name")?.value.trim();
  const phone = document.getElementById("res-phone")?.value.trim();
  const eventType = document.getElementById("res-event-type")?.value || "OTHER";
  const date = document.getElementById("res-date")?.value;
  const startTime = document.getElementById("res-time")?.value;
  const duration = Number(
    document.getElementById("res-duration")?.value || 120,
  );
  const guests = Number(document.getElementById("res-guests")?.value || 1);
  const price = document.getElementById("res-price")?.value || "0";
  const deposit = document.getElementById("res-deposit")?.value || "0";
  const paymentStatus =
    document.getElementById("res-payment-status")?.value || "UNPAID";
  const decoration =
    document.getElementById("res-decoration")?.value.trim() || "";
  const notes = document.getElementById("res-notes")?.value.trim() || "";
  const tables = Array.from(
    document.querySelectorAll("input[name='res-table']:checked"),
  ).map((box) => Number(box.value));
  if (!customerName || !phone || !date || !startTime) {
    showToast(
      "Missing details",
      "Customer name, phone, date and start time are required.",
      "warning",
    );
    return null;
  }
  if (!tables.length) {
    showToast(
      ...
      "Choose which table(s) to hold for this reservation.",
      "warning",
    );
    return null;
  }
...
function refreshReservationFormFields() {
  ...
  const checkedBefore = Array.from(
    document.querySelectorAll("input[name='res-table']:checked"),
  ).map((b) => Number(b.value));
  if (grid)
    grid.innerHTML = reservationTableCheckboxes(
      date,
      startTime,
      duration,
      editingId,
  ...
  const price = Number(document.getElementById("res-price")?.value || 0);
  const deposit = Number(document.getElementById("res-deposit")?.value || 0);
  const totalPreview = document.getElementById("res-total-preview");
  if (totalPreview)
    totalPreview.textContent = `${money(price)} · Deposit ${money(deposit)} · Balance ${money(Math.max(0, price - deposit))}`;
}
function reservationTableCheckboxes(
  dateValue,
  startTime,
  duration,
  excludeReservationId,
  ...
  const available = reservationTablesAvailableFor(
    dateValue,
    startTime,
    duration,
    excludeReservationId,
  ...
  if (!allTables.length)
    return `<p class="column-empty">${stfTables === null ? "Loading tables…" : "No tables found."}</p>`;
...
function reservationFormModal(existing) {
...
async function saveReservation(existingId) {
  const payload = reservationFormValues();
  if (!payload) return;
  try {
    if (existingId) {
      await apiJson(`/api/reservations/${existingId}/`, {
        method: "PATCH",
      ...
      showToast(
        "Reservation created",
        "Saved as Pending — confirm it once payment/details are set.",
        "success",
      );
    }
    closeModal();
...
async function reservationTransition(id, path) {
  try {
    ...
    showToast(
      `Reservation ${label}`,
      path === "confirm"
        ? "Tables are now held as Reserved."
        : "Tables have been released back to Available.",
      "success",
    );
    await loadReservations();
    if (stfTables !== null) loadStaffTables();
...
let stfNotifications = null;
async function loadNotifications() {
  try {
    stfNotifications = await apiJson("/api/notifications/");
  } catch {
    stfNotifications = [];
  }
  render();
...
async function markNotificationRead(id) {
  try {
    await apiJson(`/api/notifications/${id}/mark_read/`, { method: "POST" });
  } catch {
    /* best effort */
  }
...
async function sendNotification() {
  ...
  const roles = Array.from(
    document.querySelectorAll("input[name='notif-role']:checked"),
  ).map((box) => box.value);
  if (!title || !body) {
    showToast(
  ...
  if (!roles.length) {
    showToast(
      "Choose a recipient",
      "Pick at least one role or Customer.",
      "warning",
    );
    return;
  }
  const orderField = document.getElementById("notif-order")?.value.trim();
  const payload = {
    title,
    body,
    audience:
      roles.includes("CUSTOMER") && roles.length === 1 ? "CUSTOMER" : roles[0],
  ...
  try {
    ...
    showToast(
      "Notification sent",
      `Delivered to ${roles.map(formatStatus).join(", ")}.`,
      "success",
    );
    loadNotifications();
  } catch (error) {
...
/* ── Customer notifications (table-session scoped, read from Django API) ──── */
async function openCustomerNotifications() {
  if (!customerSessionToken) {
    showToast(
      "No active session",
      "Scan your table QR to receive notifications.",
      "warning",
    );
    return;
  }
  try {
    const response = await fetch(
      `${apiBase()}/api/notifications/?audience=CUSTOMER`,
      { headers: { "X-Table-Session": customerSessionToken } },
    );
    const list = await response.json().catch(() => []);
    const notices = Array.isArray(list) ? list : [];
    showModal(
      `<div class="modal-card notification-modal"><div class="modal-header"><div><span class="eyebrow">Your table · Live updates</span><h2>Notifications</h2></div><button class="close-button" data-action="close-modal">×</button></div><div class="notification-list">${notices.length ? notices.map((n) => `<div class="notification-row"><span>${n.read_at ? "✓" : "✦"}</span><div><strong>${escapeHtml(n.title)}</strong><p>${escapeHtml(n.body)}</p><small>${new Date(n.created_at).toLocaleString()}</small></div></div>`).join("") : `<div class="column-empty">No notifications yet — we'll update you as your order moves through the kitchen.</div>`}</div></div>`,
    );
  } catch (error) {
    showToast(
      "Could not load notifications",
...
async function loadStaffNotes() {
  try {
    stfNotes = await apiJson("/api/notes/");
  } catch (error) {
    stfError = error instanceof Error ? error.message : "Could not load notes.";
    stfNotes = [];
  }
  render();
}
async function saveRealNote() {
  const text = document.getElementById("new-note")?.value.trim();
  const audience = document.getElementById("note-audience")?.value || "MANAGER";
  const orderField = document.getElementById("note-order")?.value.trim();
  if (!text) {
    showToast("Write a note first", "The note text is required.", "warning");
    return;
  }
  ...
  try {
    ...
    showToast(
      "Note shared",
      `Visible to ${formatStatus(audience)}.`,
      "success",
    );
    loadStaffNotes();
  } catch (error) {
...
async function orderTransition(orderId, path, body) {
  try {
    const result = await apiJson(`/api/orders/${orderId}/${path}/`, {
      method: "POST",
...
async function pickupOrder(orderId) {
  ...
  if (result)
    showToast(
      "Order picked up",
      `Order ${result.number} claimed — deliver it to the table.`,
      "success",
    );
}
async function deliverOrder(orderId) {
  ...
  if (result)
    showToast(
      "Order delivered",
      `Order ${result.number} was served.`,
      "success",
    );
}
...
async function recordPayment(orderId) {
  const method =
    document.querySelector(".method-choice.active")?.dataset.paymentMethod ||
    "CASH";
  const amountField = document.getElementById("payment-amount")?.value;
  ...
  const payload = { order: orderId, amount, method };
  if (method === "INSTAPAY") {
    payload.payer_phone =
      document.getElementById("payer-phone-instapay")?.value.trim() || "";
    payload.payer_account =
      document.getElementById("payer-account")?.value.trim() || "";
    if (!payload.payer_phone || !payload.payer_account) {
      showToast(
        "Missing InstaPay details",
        "Payer phone and account number are required.",
        "warning",
      );
      return;
    }
    ...
    payload.payer_phone =
      document.getElementById("payer-phone-vodafone")?.value.trim() || "";
    if (!payload.payer_phone) {
      showToast(
        "Missing phone number",
        "Vodafone Cash requires the payer's phone number.",
        "warning",
      );
      return;
    }
  ...
  try {
    ...
    showToast(
      "Payment recorded",
      `${money(amount)} via ${formatStatus(method)}.`,
      "success",
    );
    await loadStaffOrders();
...
async function updateStaffTableStatus(id, newStatus) {
  try {
    await apiJson(`/api/tables/${id}/update_status/`, {
      method: "POST",
  ...
  if (!stored)
    return {
      ...
      availability: Object.fromEntries(
        menu.map((item) => [item.id, item.availability]),
      ),
    };
  try {
...
let customerSessionToken = safeGetStorage("pm-cafe-table-session");
let customerSessionId =
  Number(safeGetStorage("pm-cafe-table-session-id")) || null;
let customerPollTimer = null;
let editingRejectedOrderId = null;
...
function notify(title, body, tone, audience = "CUSTOMER", orderId) {
  // Real notifications come from Django (see NotificationViewSet).
  // This function now only triggers a local toast for immediate staff
  // feedback — it no longer writes fake notices to localStorage.
  void orderId; // kept for call-site compatibility
...
function sourceLabel(order) {
  return order.source === "DINE_IN_QR"
    ? `DINE-IN QR · Table ${order.tableId}`
...
function customerOrders() {
  return state.orders.filter(
    (order) => order.tableId === state.customerTableId,
  );
}
function mapApiOrderToCustomerOrder(record) {
  const items = record.items.map((item) => {
    const menuItem = menu.find(
      (candidate) => candidate.apiId === item.menu_item,
    );
    return {
...
function mediaUrl(value) {
  ...
  const apiBase = String(window.PM_API_URL || "http://127.0.0.1:8000").replace(
    /\/$/,
    "",
  );
...
function productImage(item, className = "") {
  ...
  return url
    ? `<img class="${className}" src="${escapeHtml(url)}" alt="${escapeHtml(item.name)}" loading="lazy" />`
    : "";
}
...
function itemThumb(image, name, className) {
...
function render() {
  ...
  const categoryRail = document.querySelector(".menu-section .category-row");
  ...
  if (!Array.isArray(state.tables) || !state.tables.length) {
    state.tables = initialTables.map((table) => ({
      ...table,
      sessions: [...(table.sessions || [])],
    }));
    saveState();
  ...
  if (
    state.staffRole &&
    (!authSession || authSession.role !== state.staffRole)
  ) {
    state.staffRole = null;
    saveState();
  }
...
function ensureSignature() {
  ...
  host.insertAdjacentHTML(
    "beforeend",
    '<footer class="pm-signature" aria-label="PM signature">PM</footer>',
  );
}
function renderTopbar(label, action = "") {
  return `<header class="topbar">
    ...
    <div class="topbar-actions">
      ${action}
      ${themeToggle()}
      <button class="icon-button notification-button" data-action="notifications" aria-label="Notifications">♢</button>
    </div>
  </header>`;
}
function renderScan() {
...
async function scanCustomerTable(button) {
  const table =
    initialTables.find((candidate) => candidate.id === "07") ||
    initialTables[6];
  const params = new URLSearchParams(window.location.search);
  const qrToken =
    params.get("table_token") || params.get("token") || table.token;
  button.disabled = true;
  button.innerHTML = `<span class="spinner"></span> Finding your table…`;
  try {
    const response = await fetch(
      `${apiBase()}/api/tables/scan/?token=${encodeURIComponent(qrToken)}`,
    );
    const body = await response.json().catch(() => null);
    if (!response.ok)
      throw new Error(
        apiErrorMessage(body, "The table QR code could not be verified."),
      );
    ...
    const apiTable = body.table;
    state.customerTableId = apiTable.number;
    state.tables = state.tables.map((candidate) =>
      candidate.id === apiTable.number
        ? {
            ...candidate,
            id: apiTable.number,
            label: `Table ${apiTable.number}`,
            seats: apiTable.seats,
            status: apiTable.status,
            guests: apiTable.seats,
          }
        : candidate,
    );
    saveState();
    render();
    showToast(
      `${apiTable.number} detected`,
      "Your private café session is ready.",
      "success",
    );
  } catch (error) {
    ...
    showToast(
      "Could not start table session",
      error instanceof Error ? error.message : "Use a valid table QR link.",
      "danger",
    );
  }
}
...
function tableInfo() {
  return (
    state.tables.find((table) => table.id === state.customerTableId) ||
    state.tables[6] ||
    initialTables[6]
  );
}
function renderCustomer() {
  ...
  app().innerHTML = `<main class="customer-shell">
    ${renderTopbar(`${table.label} · ${table.seats} seats`)}
...
function renderNextAction(order) {
  const messages = {
    AWAITING_PAYMENT:
      "Order received — complete payment to send it to the kitchen.",
...
function renderMenu() {
  const categoryItem = (category) =>
    `<button class="category-chip ${selectedCategory === category ? "active" : ""}" data-category="${category}"><span>${categoryIcon(category)}</span>${category}<b>${menu.filter((item) => item.category === category).length}</b></button>`;
  ...
  return `<section class="menu-section">
    ...
    ${items.length ? `<div class="product-grid">${items.map(renderProductCard).join("")}</div>` : `<div class="empty-state"><span>🏷️</span><h3>No offers right now</h3><p>New deals will show up here as soon as they are added — check back soon.</p></div>`}
    <div class="request-banner"><div class="request-icon">+</div><div><strong>Don't see what you want?</strong><span>Send a special request to the cashier. They’ll check with the kitchen.</span></div><button class="outline-button" data-action="open-request">Add request →</button></div>
  </section>`;
}
...
function renderProductCard(item) {
  ...
  return `<article class="product-card ${unavailable ? "unavailable" : ""}" data-product="${item.id}">
    <div class="product-visual visual-${(item.category || "other").toLowerCase()}">${productImage(item)}<i>${item.tags[0] || "PM Café"}</i></div>
...
function renderCustomerOrder(order) {
  ...
  return `<article class="order-card ${rejected ? "rejected" : ""}">
    ...
    <div class="order-card-bottom"><div class="mini-items">${order.items
      .slice(0, 3)
      .map((item) => itemThumb(item.image, item.name, ""))
      .join(
        "",
      )}<small>${order.items.length} item${order.items.length === 1 ? "" : "s"} · ${money(orderTotal(order))}</small></div>${order.status === "AWAITING_PAYMENT" || order.status === "APPROVED" ? `<button class="primary-button small" data-action="payment" data-order="${order.id}">Pay now →</button>` : `<span class="order-time">${order.createdAt}</span>`}</div>
  </article>`;
}
function renderBill() {
...
function cartDetails() {
  ...
  return `<div class="modal-card cart-modal"><div class="modal-header"><div><span class="eyebrow">Table ${tableInfo().id}</span><h2>Your order</h2></div><button class="close-button" data-action="close-modal">×</button></div>${state.cart.length ? `<div class="cart-lines">${state.cart.map((item, index) => `<div class="cart-line">${itemThumb(item.image, item.name, "cart-emoji")}<div class="cart-line-main"><strong>${item.name}</strong><small>${item.options.length ? item.options.join(" · ") : "Classic preparation"}${item.notes ? ` · “${escapeHtml(item.notes)}”` : ""}</small><div class="quantity"><button data-cart-change="-1" data-index="${index}">−</button><b>${item.quantity}</b><button data-cart-change="1" data-index="${index}">+</button><button class="remove-link" data-cart-remove="${index}">Remove</button></div></div><strong>${money(item.unitPrice * item.quantity)}</strong></div>`).join("")}</div>` : `<div class="empty-state compact"><span>🧺</span><h3>Your cart is empty</h3><p>Choose something from the menu, or send the kitchen a special request.</p></div>`}<div class="custom-request"><div><span class="request-icon small">+</span><div><strong>Request something not listed</strong><small>It will need cashier approval.</small></div></div><textarea id="custom-request" placeholder="e.g. Can the kitchen make a half-and-half pizza?"></textarea></div><div class="totals-box"><span>Subtotal <b>${money(bill.subtotal)}</b></span><span>Service charge <b>${money(bill.service)}</b></span><span>Tax <b>${money(bill.tax)}</b></span><strong>Grand total <b>${money(bill.total)}</b></strong></div><button class="primary-button wide" data-action="review">Review your order <span>→</span></button></div>`;
}
...
const managerSectionTitles = {
  overview: "Management overview",
  "mgr-menu": "Menu management",
  "mgr-tables": "Tables",
  "mgr-offers": "Offers",
  "mgr-employees": "Employees",
  "mgr-payroll": "Payroll",
  "mgr-attendance": "Attendance",
};
function renderStaff() {
  const roleLabel =
    state.staffRole === "KITCHEN"
      ? "Kitchen display"
      : state.staffRole === "WAITER"
        ? "Floor operations"
      ...
      : state.staffRole === "KITCHEN"
        ? [
            staffNavItem(
              ...
              "▦",
              "Kitchen queue",
              kitchenOrders.filter((order) => order.status === "APPROVED")
                .length,
            ),
            staffNavItem("notes", "✎", "Notes"),
          ].join("")
        : [
            ...
            staffNavItem(
              "orders",
              "⌁",
              state.staffRole === "WAITER" ? "Ready orders" : "Orders",
              state.staffRole === "WAITER"
                ? stfOrders.filter((o) => o.status === "READY").length
                : pendingStfOrders().length,
            ),
            staffNavItem("tables", "⌂", "Tables"),
            staffNavItem("payments", "◈", "Payments"),
            ...(state.staffRole === "CASHIER"
              ? [
                  staffNavItem(
                    "reservations",
                    "✦",
                    "Reservations",
                    cashReservations.filter((r) => r.status === "PENDING")
                      .length,
                  ),
                ]
              : []),
  ...
  const heading =
    state.staffRole === "MANAGER"
      ...
      : state.staffRole === "KITCHEN"
        ? staffSection === "notes"
          ? "Notes & handover"
          : "Kitchen queue — operational view only"
        : staffSection === "overview"
          ? `${timeGreeting()}, ${authSession?.username ? formatStatus(authSession.username) : "team"}.`
          : staffSection === "tables"
            ? "Table floor"
            : staffSection === "payments"
              ? "Payment board"
              : staffSection === "reservations"
                ? "Reservations"
                : staffSection === "notes"
                  ? "Notes & handover"
                  : state.staffRole === "WAITER"
...
function staffNavItem(section, icon, label, forcedCount) {
  const count =
    forcedCount !== undefined
      ? forcedCount
      : section === "orders" && pendingStfOrders().length
        ? pendingStfOrders().length
        : 0;
...
function pendingStfOrders() {
  return stfOrders.filter(
    (order) =>
...
function renderStaffSection() {
  if (state.staffRole === "MANAGER") {
    if (staffSection === "notes") {
      if (stfNotes === null) loadStaffNotes();
      return renderNotes();
    }
    if (staffSection === "mgr-menu") {
      if (mgrCategories === null || mgrMenuItems === null) loadManagerMenu();
      return renderManagerMenu();
    }
    if (staffSection === "mgr-tables") {
      if (mgrTables === null) loadManagerTables();
      return renderManagerTables();
    }
    if (staffSection === "mgr-offers") {
      if (mgrCategories === null || mgrMenuItems === null) loadManagerMenu();
      return renderManagerOffers();
    }
    if (staffSection === "mgr-employees") {
      if (mgrStaff === null) loadManagerStaff();
      return renderManagerEmployees();
    }
    if (staffSection === "mgr-payroll") {
      if (mgrStaff === null) loadManagerStaff();
      return renderManagerPayroll();
    }
    if (staffSection === "mgr-attendance") {
      if (mgrAttendance === null) loadManagerAttendance();
  ...
  if (staffSection === "tables") {
    if (stfTables === null) loadStaffTables();
    return renderStaffTables();
  }
...
function renderCashier() {
  const orders =
    cashierFilter === "ALL"
      ? stfOrders
      : stfOrders.filter((order) => order.status === cashierFilter);
  const sales = stfOrders
    .flatMap((order) => order.payments)
    .filter((payment) => payment.status === "PAID")
    .reduce((sum, payment) => sum + Number(payment.amount), 0);
  const activeTables = (stfTables || []).filter(
...
function renderStaffOrderReal(order) {
 ...
 * ========================================================================= */
function renderKitchenReal() {
  const visible = kitchenOrders.filter(
    (order) =>
      ["APPROVED", "PREPARING", "READY"].includes(order.status) &&
      `${order.number} ${order.table_number || ""}`
        .toLowerCase()
        .includes(kitchenSearch.toLowerCase()),
  );
  const column = (status, title, subtitle) =>
    `<div class="kitchen-column"><div class="column-head"><div><h3>${title}</h3><small>${subtitle}</small></div><b>${visible.filter((o) => o.status === status).length}</b></div>${
      visible
        .filter((o) => o.status === status)
        .map(renderKitchenCardReal)
        .join("") || `<div class="column-empty">No orders here</div>`
    }</div>`;
  const soldOut = menu.filter(
    (item) => state.availability[item.id] !== "AVAILABLE",
  );
  return `<div class="staff-view">
    ...
    <div class="availability-panel"><div><span class="eyebrow">Kitchen stock</span><h3>Item availability</h3><small>Mark sold out to hide from new orders; tap again to restock.</small></div><div class="availability-list">${
      menu
        .map((item) => {
          const isOut =
          ...
          return `<button class="stock-toggle ${isOut ? "off" : ""}" data-kitchen-sold-out="${item.id}" data-kitchen-restock="${item.id}" data-is-sold-out="${isOut}">${itemThumb(item.image, item.name, "")}<b>${item.name}</b><small>${isOut ? "Sold out — tap to restock ↺" : "Available — tap to mark sold out"}</small></button>`;
        })
        .join("") || "<span>No menu items loaded.</span>"
    }</div>${soldOut.length ? `<small class="kitchen-soldout-note">${soldOut.length} item${soldOut.length === 1 ? "" : "s"} currently sold out.</small>` : ""}</div>
  </div>`;
}
function kitchenOrderLabel(order) {
  return order.source === "DINE_IN_QR"
...
function renderKitchenCardReal(order) {
  const action =
...
async function kitchenTransition(orderId, action) {
  try {
    await apiJson(`/api/orders/${orderId}/${action}/`, { method: "POST" });
    showToast(
      action === "start" ? "Order started" : "Order marked ready",
      `Order moved to ${action === "start" ? "Preparing" : "Ready"}.`,
      "success",
    );
    loadKitchenOrders();
  } catch (error) {
...
async function kitchenMarkSoldOut(menuItemId) {
  const item = menu.find((candidate) => candidate.id === menuItemId);
  if (!item) return;
  try {
    const updated = await apiJson(`/api/menu/${item.apiId}/mark_sold_out/`, {
      method: "POST",
    });
    item.availability = "SOLD_OUT";
    showToast(
      "Marked sold out",
      `${item.name} is now hidden from new orders.`,
      "warning",
    );
    render();
  } catch (error) {
...
async function kitchenRestock(menuItemId) {
  const item = menu.find((candidate) => candidate.id === menuItemId);
  if (!item) return;
  try {
    ...
    showToast(
      "Back in stock",
      `${item.name} is now available for new orders.`,
      "success",
    );
    render();
  } catch (error) {
...
function renderTableMiniReal(table) {
  const order = stfOrders.find(
    (candidate) =>
      candidate.table_number === table.number &&
      !["COMPLETED", "CANCELLED", "REJECTED"].includes(candidate.status),
  );
  const nextStatus =
...
function renderPayments() {
  ...
  const cards = Array.from(byTable.entries())
    .map(([tableLabel, orders]) => {
 ...
 * ========================================================================= */
function reservationStatusPills() {
...
function reservationCard(r) {
  const eventLabel =
    reservationEventTypes.find((e) => e.value === r.event_type)?.label ||
    formatStatus(r.event_type);
...
function renderReservations() {
  const list =
    reservationFilter === "ALL"
      ? cashReservations
      : cashReservations.filter((r) => r.status === reservationFilter);
  const pending = cashReservations.filter((r) => r.status === "PENDING").length;
  const confirmed = cashReservations.filter(
    (r) => r.status === "CONFIRMED",
  ).length;
  const upcomingRevenue = cashReservations
...
function errorBanner() {
  return mgrError
    ? `<div class="empty-state staff-empty compact"><span>!</span><p>${escapeHtml(mgrError)}</p></div>`
    : "";
}
// ── Tables ───────────────────────────────────────────────────────────────────
function renderManagerTables() {
  if (mgrTables === null) return loadingCard("tables");
  const rows = mgrTables
    .map(
      (
        t,
      ) => `<tr><td><b>Table ${escapeHtml(t.number)}</b><small>${escapeHtml(t.location || "—")}</small></td><td>${t.seats}</td><td><span class="table-state ${statusClass(t.status)}">${formatStatus(t.status)}</span></td><td><span class="status-badge ${t.active ? "paid" : "unpaid"}">${t.active ? "ACTIVE" : "INACTIVE"}</span></td><td class="qr-token-cell"><code>${escapeHtml(t.qr_token)}</code></td><td class="table-row-actions">
    <select data-mgr-table-status="${t.id}">${["AVAILABLE", "OCCUPIED", "WAITING_FOOD", "FOOD_READY", "FINISHED", "NEEDS_CLEANING", "RESERVED"].map((s) => `<option value="${s}" ${s === t.status ? "selected" : ""}>${formatStatus(s)}</option>`).join("")}</select>
...
function addTableModal() {
  showModal(
    `<div class="modal-card note-modal"><div class="modal-header"><div><span class="eyebrow">Manager tools · live</span><h2>Add table & generate QR</h2></div><button class="close-button" data-action="close-modal">×</button></div><input id="new-table-number" placeholder="Table number, e.g. 11" /><input id="new-table-seats" type="number" min="1" value="4" /><input id="new-table-location" placeholder="Location, e.g. Terrace" /><button class="primary-button wide" data-action="save-real-table">Create table & QR</button></div>`,
  );
}
async function saveRealTable() {
  const number = document.getElementById("new-table-number")?.value.trim();
  const seats = Number(document.getElementById("new-table-seats")?.value || 0);
  const location =
    document.getElementById("new-table-location")?.value.trim() || "Main hall";
  if (!number || !seats) {
    showToast(
      "Complete table details",
      "Number and seats are required.",
      "warning",
    );
    return;
  }
  try {
    await apiJson("/api/tables/", {
      method: "POST",
    ...
    showToast(
      "Table created",
      `Table ${number} has a private QR token.`,
      "success",
    );
    loadManagerTables();
  } catch (error) {
...
async function deleteRealTable(id) {
  try {
    const result = await apiJson(`/api/tables/${id}/`, {
      method: "DELETE",
    }).catch(async () => {
    ...
    showToast(
      result?.detail ? "Table deactivated" : "Table deleted",
      result?.detail || "Removed from the floor plan.",
      "info",
    );
  } catch (error) {
    showToast(
...
async function updateRealTableStatus(id, newStatus) {
  try {
    await apiJson(`/api/tables/${id}/update_status/`, {
      method: "POST",
      body: JSON.stringify({ status: newStatus }),
...
// ── Offers ───────────────────────────────────────────────────────────────────
// An offer is a bundle (e.g. chicken + beef for one price), not a discount on another dish.
// It is stored as a normal menu item inside the "Offers" category, so the customer menu,
// cart, kitchen and billing handle it exactly like any other dish.
function findOffersCategory() {
  return (
    (mgrCategories || []).find(
      (c) =>
        c.slug === "offers" || String(c.name).trim().toLowerCase() === "offers",
    ) || null
  );
}
/** Menu items that belong to the Offers category; null until the menu data has loaded. */
function offerItems() {
  if (mgrCategories === null || mgrMenuItems === null) return null;
  const category = findOffersCategory();
  return category
    ? mgrMenuItems.filter((item) => item.category === category.id)
    : [];
}
/** Returns the Offers category, creating it (last in the category bar) the first time. */
async function ensureOffersCategory() {
  const existing = findOffersCategory();
  if (existing) return existing;
  const position =
    (mgrCategories || []).reduce(
      (highest, category) => Math.max(highest, category.position),
      0,
    ) + 1;
  const created = await apiJson("/api/categories/", {
    method: "POST",
    body: JSON.stringify({ name: "Offers", position, active: true }),
  });
  mgrCategories = [...(mgrCategories || []), created];
  return created;
}
function renderManagerOffers() {
  const items = offerItems();
  if (items === null) return loadingCard("offers");
  const category = findOffersCategory();
  const hiddenNote =
    category && !category.active
      ? `<div class="offers-inactive-note"><span>The Offers category is switched off, so customers cannot see these offers.</span><button class="outline-button" data-action="toggle-category" data-mgr-category="${category.id}" data-next="true">Activate category</button></div>`
      : "";
  const cards = items
    .map((o) => {
      const label = !o.active
        ...
        : category && !category.active
          ...
          : o.sold_out
            ? "SOLD OUT"
            : "LIVE";
      return `<article class="offer-card ${o.active ? "" : "offer-inactive"}">
    <div class="offer-card-head">${itemThumb(o.image_url || o.image, o.name, "mgr-item-thumb")}<div class="offer-card-title"><strong>${escapeHtml(o.name)}</strong><span class="offer-price-tag">${formatPrice(o.price)}</span></div><span class="status-badge ${label === "LIVE" ? "paid" : "unpaid"}">${label}</span></div>
    <p>${escapeHtml(o.description || "No description.")}</p>
    ${o.ingredients ? `<div class="offer-meta"><span>Includes: ${escapeHtml(o.ingredients)}</span></div>` : ""}
    <div class="offer-actions"><button class="outline-button" data-action="edit-menu-item" data-mgr-item="${o.id}">Edit</button><button class="outline-button" data-action="toggle-menu-item" data-mgr-item="${o.id}" data-next="${!o.active}">${o.active ? "Deactivate" : "Activate"}</button><button class="outline-button" data-action="delete-menu-item" data-mgr-item="${o.id}">Delete</button></div>
  </article>`;
    })
    .join("");
  return `<div class="staff-view">
    ${errorBanner()}
    <div class="panel-heading"><div><span class="eyebrow">Offers · live on the customer menu</span><h2>Bundles & special deals.</h2></div><button class="primary-button" data-action="add-offer">+ New offer</button></div>
    ${hiddenNote}
    <div class="offer-grid">${cards || `<div class="empty-state staff-empty"><span>✺</span><h3>No offers yet</h3><p>Create an offer and it appears for customers under the Offers category.</p></div>`}</div>
  </div>`;
}
async function newOffer() {
  try {
    const category = await ensureOffersCategory();
    // Pre-select the Offers category in the shared dish form, then restore the Menu filter.
    const previousFilter = mgrMenuFilter;
    mgrMenuFilter = category.id;
    menuItemModal();
    mgrMenuFilter = previousFilter;
  } catch (error) {
    showToast(
      "Could not start a new offer",
...
async function saveRealStaff() {
  ...
  const salary = Number(
    document.getElementById("new-staff-salary")?.value || 0,
  );
  const shiftStart = document.getElementById("new-staff-shift-start")?.value || "";
  const shiftEnd = document.getElementById("new-staff-shift-end")?.value || "";
  ...
  try {
    ...
    showToast(
      "Employee added",
      `${username} is active in the roster.`,
      "success",
    );
    loadManagerStaff();
  } catch (error) {
...
async function toggleRealStaffActive(id) {
  try {
...
function renderManagerPayroll() {
  ...
  const rows = mgrStaff
    .map((s) => {
      const recent = [...s.adjustments]
        .slice(0, 3)
        .map(
          (a) =>
            `<span class="adj-chip ${a.kind === "BONUS" ? "adj-bonus" : "adj-deduct"}">${a.kind === "BONUS" ? "+" : "−"}${money(Number(a.amount))} · ${escapeHtml(a.reason)}</span>`,
        )
        .join("");
...
function addAdjustmentModal(staffId) {
...
async function saveAdjustment(staffId) {
  ...
  try {
    await apiJson(`/api/staff/${staffId}/add_adjustment/`, {
      method: "POST",
      body: JSON.stringify({ kind, amount, reason, date, notes }),
    });
    closeModal();
    showToast(
      "Adjustment saved",
      `${kind === "BONUS" ? "Bonus" : "Deduction"} of ${money(amount)} recorded.`,
      "success",
    );
    loadManagerStaff();
  } catch (error) {
...
function renderManagerAttendance() {
  if (mgrStaff === null || mgrAttendance === null)
    return loadingCard("attendance");
...
async function markAttendance(staffId, status) {
  try {
    await apiJson("/api/attendance/", {
      method: "POST",
      body: JSON.stringify({ staff: staffId, date: mgrAttendanceDate, status }),
    });
...
function renderManagerMenu() {
  if (mgrCategories === null || mgrMenuItems === null)
    return loadingCard("menu");
  ...
  const categoryRows = categoryList
    .map(
      (category) => `<tr class="${category.active ? "" : "mgr-row-inactive"}">
  ...
  const itemRows = visible
    .map(
      (item) => `<tr class="${item.active ? "" : "mgr-row-inactive"}">
  ...
  const chips = [
    ...
    ...categoryList.map(
      (category) =>
        `<button class="category-chip ${mgrMenuFilter === category.id ? "active" : ""}" data-mgr-menu-filter="${category.id}">${escapeHtml(category.name)}<b>${countIn(category.id)}</b></button>`,
    ),
  ].join("");
...
function readCategoryDraft() {
  return {
    ...
    position: Number(document.getElementById("cat-position")?.value ?? 0) || 0,
...
function categoryModal(existing, draft) {
  ...
  const currentImage = pendingCategoryImagePreviewUrl
    ...
    : existing?.image
      ? `<div class="mgr-current-image">${itemThumb(existing.image, existing.name, "mgr-item-thumb")}<small>Current photo — choose a file below only to replace it.</small></div>`
      : "";
...
async function saveCategory(id) {
  ...
  const position = Number(document.getElementById("cat-position")?.value || 0);
  const rawSlug =
    document.getElementById("cat-slug")?.value.trim().toLowerCase() || "";
  const slug = rawSlug.replace(/[^a-z0-9_-]+/g, "-").replace(/^-+|-+$/g, "");
  const active = document.getElementById("cat-active")?.checked ?? true;
  if (!name) {
    showToast("Name required", "Give the category a name.", "warning");
    return;
  }
  if (!Number.isFinite(position) || position < 0) {
    showToast("Check the position", "Position must be 0 or higher.", "warning");
    return;
  }
  const fields = { name, position: Math.floor(position), active };
  if (slug) fields.slug = slug;
  const rawFile = document.getElementById("cat-image")?.files?.[0];
  const imageBlob =
    pendingCategoryImageBlob ||
    (rawFile && rawFile.size <= 8 * 1024 * 1024 ? rawFile : null);
  let body;
  if (imageBlob) {
...
async function toggleCategoryActive(id, next) {
  try {
    ...
    showToast(
      ...
      next
        ? "Its dishes are back on the customer menu."
        : "Its dishes are hidden from the customer menu.",
      "info",
    );
    await refreshManagerMenu();
  } catch (error) {
...
function confirmDeleteModal(kind, id) {
  const category =
    kind === "category" ? mgrCategories?.find((c) => c.id === id) : undefined;
  const item =
    kind === "menu-item" ? mgrMenuItems?.find((i) => i.id === id) : undefined;
...
async function deleteCategory(id) {
  try {
    const result = await apiJson(`/api/categories/${id}/`, {
      method: "DELETE",
    });
    closeModal();
    showToast(
      result?.detail ? "Category deactivated" : "Category deleted",
      result?.detail || "Removed from the menu.",
      "info",
    );
...
function menuItemModal(existing, draft) {
  ...
  const selectedCategoryId =
    draft?.category ||
    existing?.category ||
    (typeof mgrMenuFilter === "number" ? mgrMenuFilter : categoryList[0].id);
  const offersCategory = findOffersCategory();
  const isOffer = !!offersCategory && selectedCategoryId === offersCategory.id;
  const options = categoryList
    .map(
      (category) =>
        `<option value="${category.id}" ${category.id === selectedCategoryId ? "selected" : ""}>${escapeHtml(category.name)}${category.active ? "" : " (inactive)"}</option>`,
    )
    .join("");
  ...
  showModal(`<div class="modal-card note-modal mgr-modal">
    <div class="modal-header"><div><span class="eyebrow">Manager tools · live</span><h2>${existing ? (isOffer ? "Edit offer" : "Edit menu item") : isOffer ? "Create offer" : "Add menu item"}</h2></div><button class="close-button" data-action="close-modal">×</button></div>
    <label class="field-label">Name<input id="item-name" placeholder="${isOffer ? "e.g. Chicken & beef combo" : "e.g. Chocolate lava cake"}" maxlength="140" value="${escapeHtml(name)}" /></label>
    <div class="form-row">
      <label class="field-label">Category<select id="item-category">${options}</select></label>
      <label class="field-label">${isOffer ? "Offer price (EGP)" : "Price (EGP)"}<input id="item-price" type="number" min="0" step="0.01" value="${escapeHtml(priceValue)}" placeholder="e.g. 85" /></label>
    </div>
    <label class="field-label">Description<textarea id="item-description" placeholder="${isOffer ? "What the customer gets, shown on the offer card" : "Short blurb shown on the menu card"}">${escapeHtml(description)}</textarea></label>
    <label class="field-label">${isOffer ? "Includes" : "Ingredients"}<textarea id="item-ingredients" placeholder="${isOffer ? "e.g. 2 chicken pieces, beef portion, fries, soft drink" : "Comma-separated, e.g. flour, cocoa, butter"}">${escapeHtml(ingredients)}</textarea></label>
    ...
    ${isOffer ? "" : `<label class="check-option"><input type="checkbox" id="item-meal" ${meal ? "checked" : ""} /><span>Complete meal</span></label>`}
    <div class="mgr-item-options-row">${optionsAddonsHint}</div>
    <button class="primary-button wide" data-action="save-menu-item" data-mgr-item="${existing?.id || ""}">${existing ? "Save changes" : isOffer ? "Create offer" : "Create menu item"}</button>
  </div>`);
}
async function saveMenuItem(id) {
  ...
  const rawFile = document.getElementById("item-image")?.files?.[0];
  if (!name) {
    showToast("Name required", "Give the dish a name.", "warning");
    return;
  }
  if (!category) {
    showToast("Category required", "Choose a category.", "warning");
    return;
  }
  if (priceText === "" || !Number.isFinite(price) || price < 0) {
    showToast("Check the price", "Enter a price of 0 or more.", "warning");
    return;
  }
  if (!description) {
    showToast(
      "Description required",
      "Add a short description for the menu card.",
      "warning",
    );
    return;
  }
  const fields = {
    ...
    active: checked("item-active"),
    sold_out: checked("item-sold-out"),
    is_complete_meal: checked("item-meal"),
  };
  ...
  try {
    await apiJson(id ? `/api/menu/${id}/` : "/api/menu/", {
      method: id ? "PATCH" : "POST",
      body,
    });
    closeModal();
...
function itemOptionsModal(item) {
  const current =
    mgrMenuItems?.find((candidate) => candidate.id === item.id) || item;
  const optionRows = current.options.length
    ? current.options
        .map(
          (o) => `<tr class="${o.active ? "" : "mgr-row-inactive"}">
  ...
  const addonRows = current.addons.length
    ? current.addons
        .map(
          (a) => `<tr class="${a.active ? "" : "mgr-row-inactive"}">
...
async function addItemOption(itemId) {
  const name = document.getElementById("new-option-name")?.value.trim();
  const delta = Number(document.getElementById("new-option-delta")?.value || 0);
  if (!name) {
    showToast("Name required", "Give the option a name.", "warning");
    return;
  }
  if (!Number.isFinite(delta)) {
    showToast("Check the price change", "Enter a valid number.", "warning");
    return;
  }
  try {
    await apiJson("/api/menu-options/", {
      method: "POST",
      body: JSON.stringify({
        item: itemId,
        name,
        price_delta: Math.round(delta * 100) / 100,
      }),
    });
...
async function toggleItemOption(optionId, next, itemId) {
  try {
    await apiJson(`/api/menu-options/${optionId}/`, {
      method: "PATCH",
      body: JSON.stringify({ active: next }),
    });
    await refreshManagerMenu();
...
async function deleteItemOption(optionId, itemId) {
  try {
...
async function addItemAddon(itemId) {
  const name = document.getElementById("new-addon-name")?.value.trim();
  const price = Number(document.getElementById("new-addon-price")?.value || 0);
  if (!name) {
    showToast("Name required", "Give the add-on a name.", "warning");
    return;
  }
  ...
  try {
    ...
    showToast(
      "Could not add add-on",
      errorMessage(error, "Try again."),
      "danger",
    );
  }
}
async function toggleItemAddon(addonId, next, itemId) {
  try {
    await apiJson(`/api/menu-addons/${addonId}/`, {
      method: "PATCH",
      body: JSON.stringify({ active: next }),
    });
    await refreshManagerMenu();
...
async function deleteItemAddon(addonId, itemId) {
  try {
...
function openCropperForFile(file, target) {
  if (!file.type.startsWith("image/")) {
    showToast("Not an image", "Please choose an image file.", "warning");
    return;
  }
  ...
  img.onerror = () => {
    ...
    showToast("Could not load photo", "Try a different image file.", "danger");
  };
  img.src = objectUrl;
}
function cropperEffectiveSize() {
  if (!cropper) return { w: 0, h: 0 };
  const swapped = cropper.rotation % 180 !== 0;
  return swapped
    ? { w: cropper.naturalH, h: cropper.naturalW }
    : { w: cropper.naturalW, h: cropper.naturalH };
}
function recomputeCropperBaseScale() {
  if (!cropper) return;
...
function exportCropperBlob() {
  return new Promise((resolve) => {
    ...
    ctx.save();
    ctx.translate(
      out.width / 2 + cropper.offsetX * EXPORT_SCALE,
      out.height / 2 + cropper.offsetY * EXPORT_SCALE,
    );
    ctx.rotate((cropper.rotation * Math.PI) / 180);
    ctx.scale(
      cropper.baseScale * cropper.zoom * EXPORT_SCALE,
      cropper.baseScale * cropper.zoom * EXPORT_SCALE,
    );
    ctx.drawImage(cropper.img, -cropper.naturalW / 2, -cropper.naturalH / 2);
    ctx.restore();
...
function reopenCropperParentModal() {
  if (!cropper) {
    closeModal();
    return;
  }
  ...
  if (target === "item") {
    ...
    const existing = itemDraft.existingId
      ? mgrMenuItems?.find((candidate) => candidate.id === itemDraft.existingId)
      : undefined;
    menuItemModal(existing, itemDraft);
  } else {
    const catDraft = draft;
    const existing = catDraft.existingId
      ? mgrCategories?.find((candidate) => candidate.id === catDraft.existingId)
      : undefined;
    categoryModal(existing, catDraft);
  }
}
async function saveCropperResult() {
  if (!cropper) return;
  const blob = await exportCropperBlob();
  if (!blob) {
    showToast("Could not process photo", "Try a different image.", "danger");
    return;
  }
...
async function toggleMenuItemActive(id, next) {
  try {
    await apiJson(`/api/menu/${id}/`, {
      method: "PATCH",
      body: JSON.stringify({ active: next }),
    });
    showToast(
      next ? "Item activated" : "Item deactivated",
      next ? "Visible on the customer menu." : "Hidden from the customer menu.",
      "info",
    );
    await refreshManagerMenu();
  } catch (error) {
...
async function toggleMenuItemSoldOut(id) {
  try {
    const updated = await apiJson(`/api/menu/${id}/toggle_availability/`, {
      method: "POST",
    });
    showToast(
      updated.sold_out ? "Marked sold out" : "Back in stock",
      updated.name,
      "info",
    );
    await refreshManagerMenu();
...
async function deleteMenuItem(id) {
  try {
    const result = await apiJson(`/api/menu/${id}/`, { method: "DELETE" });
    closeModal();
    showToast(
      result?.detail ? "Item deactivated" : "Item deleted",
      result?.detail || "Removed from the menu.",
      "info",
    );
    await refreshManagerMenu();
...
async function saveEditedTable(id) {
  const number = document.getElementById("edit-table-number")?.value.trim();
  const seats = Number(document.getElementById("edit-table-seats")?.value || 0);
  const location = document.getElementById("edit-table-location")?.value.trim();
  const active = document.getElementById("edit-table-active")?.checked ?? true;
  if (!number || !location) {
    showToast(
      "Complete table details",
      "Number and location are required.",
      "warning",
    );
    return;
  }
  if (!Number.isInteger(seats) || seats < 1) {
    showToast("Check the seats", "A table needs at least 1 seat.", "warning");
    return;
  }
  try {
    await apiJson(`/api/tables/${id}/`, {
      method: "PATCH",
      body: JSON.stringify({ number, seats, location, active }),
    });
    closeModal();
    showToast(
      "Table updated",
      `Table ${number} · ${seats} seats · ${location}`,
      "success",
    );
    loadManagerTables();
  } catch (error) {
...
// ── Employees: edit ──────────────────────────────────────────────────────────
function employmentLabel(member) {
...
async function saveEditedStaff(id) {
  const role = document.getElementById("edit-staff-role")?.value;
  const salaryText = document.getElementById("edit-staff-salary")?.value.trim();
  const salary = Number(salaryText);
  const phone = document.getElementById("edit-staff-phone")?.value.trim() || "";
  const startDate = document.getElementById("edit-staff-start")?.value || "";
  const endDate = document.getElementById("edit-staff-end")?.value || "";
  const shiftStart = document.getElementById("edit-staff-shift-start")?.value || "";
  const shiftEnd = document.getElementById("edit-staff-shift-end")?.value || "";
  const active = document.getElementById("edit-staff-active")?.checked ?? true;
  if (salaryText === "" || !Number.isFinite(salary) || salary < 0) {
    showToast("Check the salary", "Enter a salary of 0 or more.", "warning");
    return;
  }
...
function hourlyChart(distribution) {
  ...
  const quiet = distribution.reduce(
    (best, row) => (row.count < best.count ? row : best),
    distribution[0],
  );
...
function renderManagerOverview() {
  ...
  const occupied =
    mgrTables?.filter((t) => t.status !== "AVAILABLE").length ?? null;
  const liveOfferList = offerItems();
  const activeOffers = liveOfferList
    ? liveOfferList.filter((o) => o.active && !o.sold_out).length
    : null;
  ...
  const livePanels = a
    ...
    : `<div class="empty-state staff-empty compact">${mgrAnalyticsError ? `<span>!</span><p>${escapeHtml(mgrAnalyticsError)}</p><button class="outline-button" data-action="refresh-analytics">Retry</button>` : `<span class="spinner"></span><p>Loading live analytics…</p>`}</div>`;
...
function openProduct(item) {
  ...
  const extraOptions =
    item.category === "Pizza"
      ? ["Large size", "Thin crust", "Extra cheese", "Mushrooms"]
...
function loginModal() {
  showModal(
    `<div class="modal-card login-modal"><button class="close-button" data-action="close-modal">×</button><span class="brand-mark large">✦</span><span class="eyebrow">Private workspace</span><h2>Welcome back, team.</h2><p>Choose a role to open the right tools for this shift.</p><div class="role-select">${["CASHIER", "KITCHEN", "WAITER", "MANAGER"].map((role, index) => `<button class="${index === 0 ? "active" : ""}" data-login-role="${role}"><span>${role === "CASHIER" ? "◈" : role === "KITCHEN" ? "♨" : role === "WAITER" ? "⌂" : "✦"}</span>${formatStatus(role)}</button>`).join("")}</div><input id="login-username" placeholder="Username" autocomplete="username" /><input id="login-password" type="password" placeholder="Password" autocomplete="current-password" /><button class="primary-button wide" data-action="login">Enter workspace →</button><small>Every staff role signs in against the live Django account and receives only its permitted workspace.</small></div>`,
  );
}
function rejectionModal(orderId) {
  showModal(
    `<div class="modal-card rejection-modal"><div class="modal-header"><div><span class="eyebrow">Order #${orderId}</span><h2>Why can’t we make this?</h2><p>This reason will be shown to the customer so they can fix their order.</p></div><button class="close-button" data-action="close-modal">×</button></div><div class="reason-grid">${rejectionReasons.map((reason, index) => `<label class="reason-option"><input type="radio" name="reject-reason" value="${reason}" ${index === 0 ? "checked" : ""} /><span>${reason}</span></label>`).join("")}</div><textarea id="rejection-note" placeholder="Additional note (optional)"></textarea><button class="reject-button wide" data-action="confirm-reject" data-order="${orderId}">Reject order</button></div>`,
  );
}
function paymentModal(orderId) {
  const apiOrder = stfOrders.find(
    (candidate) => String(candidate.id) === orderId,
  );
  if (apiOrder) {
    const paid = apiOrder.payments
      .filter((payment) => payment.status === "PAID")
      .reduce((sum, payment) => sum + Number(payment.amount), 0);
    const remaining = Math.max(0, Number(apiOrder.total) - paid);
    showModal(
      `<div class="modal-card payment-modal"><div class="modal-header"><div><span class="eyebrow">Order #${escapeHtml(apiOrder.number)} · Table ${apiOrder.table_number || "—"}</span><h2>Record payment</h2><p>Payment is saved to the live café API.</p></div><button class="close-button" data-action="close-modal">×</button></div><div class="payment-amount"><span>Remaining</span><strong>${money(remaining)}</strong></div><label class="payment-field-label">Amount<input id="payment-amount" type="number" min="0.01" step="0.01" value="${remaining.toFixed(2)}" /></label><div class="payment-methods">${[
        ["CASH", "₤", "Cash", "Cash in hand"],
        ["CARD", "▣", "Card", "Use hosted checkout for customers"],
        ["VODAFONE_CASH", "⌁", "Vodafone Cash", "Payer phone required"],
        ["INSTAPAY", "↗", "InstaPay", "Phone and account required"],
      ]
        .map(
          (method, index) =>
            `<button class="method-choice ${index === 0 ? "active" : ""}" data-payment-method="${method[0]}"><span>${method[1]}</span><b>${method[2]}</b><small>${method[3]}</small></button>`,
        )
        .join(
          "",
        )}</div><div class="payment-method-fields"><div data-payment-fields="CASH"><small class="modal-foot">Cash payment will be recorded immediately.</small></div><div data-payment-fields="CARD" hidden><small class="modal-foot">Online card payments use Stripe Checkout. No card data is entered here.</small></div><div data-payment-fields="VODAFONE_CASH" hidden><label class="payment-field-label">Payer phone number <input id="payer-phone-vodafone" inputmode="tel" placeholder="01XXXXXXXXX" /></label></div><div data-payment-fields="INSTAPAY" hidden><label class="payment-field-label">Payer phone number <input id="payer-phone-instapay" inputmode="tel" placeholder="01XXXXXXXXX" /></label><label class="payment-field-label">Payer account number <input id="payer-account" placeholder="InstaPay account or wallet number" /></label></div></div><button class="primary-button wide" data-action="record-payment" data-order="${apiOrder.id}">Record ${money(remaining)} <span>→</span></button><small class="modal-foot">Cash and wallet payments remain manual; Stripe status comes from its verified webhook.</small></div>`,
    );
    return;
  }
  ...
  if (!order) return;
  showModal(
    `<div class="modal-card payment-modal"><div class="modal-header"><div><span class="eyebrow">Order #${order.id} · Table ${order.tableId}</span><h2>Pay securely</h2><p>You will finish payment on Stripe's hosted checkout page.</p></div><button class="close-button" data-action="close-modal">×</button></div><div class="payment-amount"><span>Total to pay</span><strong>${money(orderTotal(order))}</strong></div><div class="secure-payment-note">Your card number and CVV are entered only on Stripe. PM Café never receives or stores them.</div><button class="primary-button wide" data-action="start-checkout" data-order="${orderId}">Continue to secure checkout <span>↗</span></button><div class="demo-payment-divider"><span>or</span></div><button class="outline-button wide demo-pay-btn" data-action="demo-payment" data-order="${orderId}">⚡ Simulate payment <small>(Demo)</small></button><small class="modal-foot demo-note">Demo payment records gateway=DEMO and is flagged in cashier/manager reports. Disable with DEMO_PAYMENTS_ENABLED=0 before going live.</small></div>`,
  );
}
...
function addProductToCart() {
  ...
  const options = Array.from(
    document.querySelectorAll(".product-modal input:checked"),
  ).map((input) => input.value);
  const notes = document.getElementById("product-note")?.value || "";
  const existing = state.cart.find(
    (item) =>
      item.menuItemId === activeProduct?.id &&
      item.options.join() === options.join() &&
      item.notes === notes,
  );
  if (existing) existing.quantity += modalQuantity;
    ...
    state.cart = [
      ...state.cart,
      {
        ...
        unitPrice: priceForChoices(activeProduct, options),
  ...
  showToast(
    "Added to your order",
    `${activeProduct.name} is in the cart.`,
    "success",
  );
}
/* ── Customer special request (no cart items required) ───────────────────── */
function openRequestModal() {
  if (!customerSessionToken || !customerSessionId) {
    showToast(
      "Scan your table QR first",
      "A table session is required to send requests.",
      "warning",
    );
    return;
  }
  showModal(
    `<div class="modal-card cart-modal"><div class="modal-header"><div><span class="eyebrow">Special request</span><h2>Ask the cashier</h2></div><button class="close-button" data-action="close-modal">×</button></div><div class="custom-request" style="margin-top:0"><div><span class="request-icon small">+</span><div><strong>What would you like?</strong><small>Tell the cashier — they'll check with the kitchen and get back to you.</small></div></div><textarea id="special-request-text" placeholder="e.g. Can we get a half-and-half pizza? Any nut-free options?" rows="4" style="min-height:100px"></textarea></div><div class="review-actions" style="margin-top:16px"><button class="outline-button" data-action="close-modal">Cancel</button><button class="primary-button" data-action="send-request">Send request <span>→</span></button></div></div>`,
  );
  setTimeout(
    () => document.getElementById("special-request-text")?.focus(),
    50,
  );
}
async function submitCustomerRequest() {
  const text = document.getElementById("special-request-text")?.value.trim();
  if (!text) {
    showToast("Empty request", "Type your request before sending.", "warning");
    return;
  }
  if (!customerSessionToken || !customerSessionId) {
    showToast(
  ...
  const button = document.querySelector("[data-action='send-request']");
  if (button) {
    button.disabled = true;
  ...
  try {
    ...
    const response = await fetch(`${apiBase()}/api/orders/`, {
      ...
      headers: {
        ...
        "X-Table-Session": customerSessionToken,
      },
      body: JSON.stringify(payload),
    });
    const body = await response.json().catch(() => null);
    if (!response.ok)
      throw new Error(apiErrorMessage(body, "The request could not be sent."));
    const order = mapApiOrderToCustomerOrder(body);
    ...
    showToast(
      "Request sent!",
      "The cashier will see it right away.",
      "success",
    );
  } catch (error) {
    if (button) {
...
async function submitOrder(request) {
  ...
  if (!state.cart.length && !decodedRequest.trim()) {
    showToast(
      "Nothing to submit",
      "Add an item or type a special request first.",
      "warning",
    );
    return;
  }
  if (!customerSessionToken || !customerSessionId) {
    showToast(
      "Scan your table QR first",
      "The order must be linked to an active table session.",
      "warning",
    );
    return;
  }
  ...
  if (items.length && items.some((item) => !item.menu_item)) {
    showToast(
      "Menu is still loading",
      "Please wait for the live menu before submitting.",
      "warning",
    );
    return;
  }
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        apiErrorMessage(body, "The order could not be submitted."),
      );
    const order = mapApiOrderToCustomerOrder(body);
    state.orders = [
      order,
      ...state.orders.filter((candidate) => candidate.id !== order.id),
    ];
    state.cart = [];
    editingRejectedOrderId = null;
    state.tables = state.tables.map((table) =>
      table.id === order.tableId ? { ...table, status: "WAITING_FOOD" } : table,
    );
    saveState();
    closeModal();
    customerView = "orders";
    render();
    showToast(
      isResubmit ? "Order updated" : "Order received",
      `Order #${order.id} — complete payment to send it to the kitchen.`,
      "success",
    );
  } catch (error) {
    if (button) {
...
/* ── Demo payment (simulation, no real money) ────────────────────────────── */
async function submitDemoPayment(orderId) {
  if (!customerSessionToken) {
    showToast("No active session", "Scan your table QR first.", "warning");
    return;
  }
  const order = state.orders.find((o) => o.id === orderId);
  if (!order) {
    showToast("Order not found", "Refresh and try again.", "danger");
    return;
  }
  const btn = document.querySelector("[data-action='demo-payment']");
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `<span class="spinner"></span> Processing…`;
  }
  try {
    const backendId = order.backendId || order.apiId || orderId;
    const response = await fetch(
      `${apiBase()}/api/orders/${backendId}/demo_pay/`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    const body = await response.json().catch(() => null);
    if (!response.ok)
      throw new Error(apiErrorMessage(body, "Demo payment failed."));
    const updated = mapApiOrderToCustomerOrder(body);
    ...
    showToast(
      "Payment simulated ✓",
      `Order confirmed — the kitchen will start preparing now.`,
      "success",
    );
  } catch (error) {
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = `⚡ Simulate payment <small>(Demo)</small>`;
    }
    showToast(
      "Demo payment failed",
      error instanceof Error ? error.message : "Try again.",
...
async function startOnlineCheckout(orderId) {
  const order = state.orders.find((candidate) => candidate.id === orderId);
  const button = document.querySelector("[data-action='start-checkout']");
  if (!order?.backendId || !customerSessionToken) {
    showToast(
      "Checkout unavailable",
      "Refresh your table session and try again.",
      "warning",
    );
    return;
  }
  if (button) {
    button.disabled = true;
    button.innerHTML = `<span class="spinner"></span> Opening secure checkout…`;
  }
  try {
    ...
    if (!response.ok || !body?.checkout_url)
      throw new Error(
        apiErrorMessage(body, "Online checkout could not be started."),
      );
    window.location.assign(body.checkout_url);
  } catch (error) {
    if (button) {
      button.disabled = false;
      button.innerHTML = "Continue to secure checkout <span>↗</span>";
    }
    showToast(
...
function printReceipt(orderId) {
  ...
  if (!order) {
    showToast(
      "Receipt unavailable",
      "Refresh the order board and try again.",
      "warning",
    );
    return;
  }
  ...
  if (!receiptWindow) {
    showToast(
      ...
      "The browser blocked the printable receipt window.",
      "warning",
    );
    return;
  }
  const rows = order.items
    .map(
      (item) =>
        `<tr><td>${item.quantity}× ${escapeHtml(item.name_snapshot)}${item.options.length ? `<small>${item.options.map(escapeHtml).join(" · ")}</small>` : ""}</td><td>${money(Number(item.unit_price) * item.quantity)}</td></tr>`,
    )
    .join("");
...
function updatePaymentFields() {
  const method =
    document.querySelector(".method-choice.active")?.dataset.paymentMethod ||
    "CASH";
  document.querySelectorAll("[data-payment-fields]").forEach((field) => {
...
function handleClick(event) {
  ...
  if (action === "scan") {
    ...
    scanCustomerTable(actionElement);
   ...
   * always clear it, so the PM logo reliably returns to the entry page. */ else if (
    action === "home" ||
    action === "staff-home"
  ) {
    state.staffRole = null;
    ...
    if (state.staffRole) openStaffNotifications();
    else openCustomerNotifications();
  } else if (action === "customer-notices")
    showModal(
      `<div class="modal-card notification-modal"><div class="modal-header"><div><span class="eyebrow">Live activity</span><h2>Notifications</h2></div><button class="close-button" data-action="close-modal">×</button></div><div class="notification-list">${state.notices
        .filter((notice) => notice.audience === "CUSTOMER")
        .slice(0, 12)
        .map(
          (notice) =>
            `<div class="notification-row ${notice.tone}"><span>${notice.tone === "success" ? "✓" : "✦"}</span><div><strong>${escapeHtml(notice.title)}</strong><p>${escapeHtml(notice.body)}</p><small>${notice.createdAt}</small></div></div>`,
        )
        .join("")}</div></div>`,
    );
    ...
    const item = menu.find(
      (candidate) => candidate.id === actionElement?.dataset.product,
    );
    if (item) openProduct(item);
  } else if (action === "add-product") addProductToCart();
  else if (action === "open-request") openRequestModal();
  else if (action === "send-request") submitCustomerRequest();
  else if (action === "cart") showModal(cartDetails());
  else if (action === "review") {
    const requestText =
      document.getElementById("custom-request")?.value.trim() || "";
    if (state.cart.length || requestText) showModal(reviewDetails());
    else
      showToast(
        "Nothing to submit",
        "Add an item or type a special request first.",
    ...
    submitOrder(actionElement?.dataset.request || "");
  else if (action === "refresh-menu") refreshMenu();
  else if (action === "approve")
    approveOrder(Number(actionElement.dataset.order));
  else if (action === "reject")
    rejectionModal(actionElement.dataset.order || "");
  else if (action === "confirm-reject")
    confirmRejectOrder(Number(actionElement.dataset.order));
  else if (action === "fix-order") {
    const order = state.orders.find(
      (candidate) => candidate.id === actionElement.dataset.order,
    );
    if (order?.backendId) {
      ...
      showToast(
        "Order ready to edit",
        "The rejected items are back in your cart. Submit to resubmit this order.",
        "warning",
      );
    }
  } else if (action === "payment")
    paymentModal(actionElement.dataset.order || "");
  else if (action === "record-payment")
    recordPayment(Number(actionElement.dataset.order));
  else if (action === "demo-payment")
    submitDemoPayment(actionElement.dataset.order || "");
  else if (action === "start-checkout")
    startOnlineCheckout(actionElement.dataset.order || "");
  else if (action === "start-preparing") {
    updateOrder(actionElement.dataset.order || "", { status: "PREPARING" });
    notify(
      "Kitchen started",
      `Order #${actionElement.dataset.order} is being prepared.`,
      "info",
      "CUSTOMER",
    ...
    if (order) {
      notify(
        "Order ready",
        `Table ${order.tableId} order is ready. Please pick it up.`,
        "success",
        "STAFF",
        orderId,
      );
      notify(
        "Your order is ready",
        "A waiter is being notified to bring it to you.",
        "success",
        "CUSTOMER",
        orderId,
      );
    ...
    const area =
      document.getElementById("new-table-area")?.value || "Main hall";
    if (
      !number ||
      state.tables.some((table) => table.id === number.padStart(2, "0"))
    ) {
      showToast(
        "Choose a new table number",
        "That table already exists or is empty.",
        "warning",
      );
      return;
    }
    ...
    state.tables = [
      ...state.tables,
      {
        ...
        guests: 0,
        status: "AVAILABLE",
        token: `pm-t${id}-${Date.now().toString().slice(-3)}`,
        area,
        sessions: [],
      },
    ];
    ...
    showToast(
      "Table created",
      `Table ${id} has a private QR token.`,
      "success",
    );
    ...
    closeModal();
    render();
    showToast("Employee added", `${name} is active in the roster.`, "success");
  } else if (action === "toggle-staff") {
    state.staff = state.staff.map((member) =>
      member.id === actionElement.dataset.staff
   ...
   * previously did nothing at all. */ else if (action === "kitchen-start")
    kitchenTransition(Number(actionElement.dataset.order), "start");
  else if (action === "kitchen-ready")
    kitchenTransition(Number(actionElement.dataset.order), "ready");
  else if (action === "kitchen-note")
    kitchenNoteModal(Number(actionElement.dataset.order));
  else if (action === "kitchen-save-note")
    saveKitchenNote(Number(actionElement.dataset.order));
  else if (action === "add-real-table") addTableModal();
  else if (action === "save-real-table") saveRealTable();
  else if (action === "delete-table")
    deleteRealTable(Number(actionElement.dataset.table));
  else if (action === "add-offer") newOffer();
  else if (action === "add-real-staff") addStaffModal();
  else if (action === "save-real-staff") saveRealStaff();
  else if (action === "toggle-real-staff")
    toggleRealStaffActive(Number(actionElement.dataset.staff));
  else if (action === "add-adjustment")
    addAdjustmentModal(Number(actionElement.dataset.staff));
  else if (action === "save-adjustment")
    saveAdjustment(Number(actionElement.dataset.staff));
  else if (action === "mark-attendance")
    markAttendance(
      Number(actionElement.dataset.staff),
      actionElement.dataset.status || "PRESENT",
    );
  /* Part 3A (menu / table edit / employee edit / analytics): every branch below calls a
   * real Django endpoint through apiJson() — nothing here touches localStorage. */ else if (
    action === "add-category"
  )
    categoryModal();
  else if (action === "edit-category") {
    const c = mgrCategories?.find(
      (x) => x.id === Number(actionElement.dataset.mgrCategory),
    );
    if (c) categoryModal(c);
  } else if (action === "save-category")
    saveCategory(Number(actionElement.dataset.mgrCategory) || undefined);
  else if (action === "toggle-category")
    toggleCategoryActive(
      Number(actionElement.dataset.mgrCategory),
      actionElement.dataset.next === "true",
    );
  else if (action === "delete-category")
    confirmDeleteModal("category", Number(actionElement.dataset.mgrCategory));
  else if (action === "confirm-delete-category")
    deleteCategory(Number(actionElement.dataset.mgrCategory));
  else if (action === "add-menu-item") menuItemModal();
  else if (action === "edit-menu-item") {
    const i = mgrMenuItems?.find(
      (x) => x.id === Number(actionElement.dataset.mgrItem),
    );
    if (i) menuItemModal(i);
  } else if (action === "save-menu-item")
    saveMenuItem(Number(actionElement.dataset.mgrItem) || undefined);
  else if (action === "toggle-menu-item")
    toggleMenuItemActive(
      Number(actionElement.dataset.mgrItem),
      actionElement.dataset.next === "true",
    );
  else if (action === "toggle-menu-sold-out")
    toggleMenuItemSoldOut(Number(actionElement.dataset.mgrItem));
  else if (action === "delete-menu-item")
    confirmDeleteModal("menu-item", Number(actionElement.dataset.mgrItem));
  else if (action === "confirm-delete-menu-item")
    deleteMenuItem(Number(actionElement.dataset.mgrItem));
  else if (action === "manage-item-options") {
    const i = mgrMenuItems?.find(
      (x) => x.id === Number(actionElement.dataset.mgrItem),
    );
    if (i) itemOptionsModal(i);
  } else if (action === "add-item-option")
    addItemOption(Number(actionElement.dataset.mgrItem));
  else if (action === "toggle-item-option")
    toggleItemOption(
      Number(actionElement.dataset.option),
      actionElement.dataset.next === "true",
      Number(actionElement.dataset.mgrItem),
    );
  else if (action === "delete-item-option")
    deleteItemOption(
      Number(actionElement.dataset.option),
      Number(actionElement.dataset.mgrItem),
    );
  else if (action === "add-item-addon")
    addItemAddon(Number(actionElement.dataset.mgrItem));
  else if (action === "toggle-item-addon")
    toggleItemAddon(
      Number(actionElement.dataset.addon),
      actionElement.dataset.next === "true",
      Number(actionElement.dataset.mgrItem),
    );
  else if (action === "delete-item-addon")
    deleteItemAddon(
      Number(actionElement.dataset.addon),
      Number(actionElement.dataset.mgrItem),
    );
  else if (action === "cropper-rotate") {
    if (cropper) {
  ...
  } else if (action === "cropper-shape") {
    if (cropper) {
      cropper.shape = actionElement.dataset.shape || "square";
      cropper.offsetX = 0;
      cropper.offsetY = 0;
      recomputeCropperBaseScale();
      openCropperModal();
    }
  } else if (action === "cropper-reset") {
    if (cropper) {
      cropper.rotation = 0;
      cropper.zoom = 1;
      cropper.offsetX = 0;
      cropper.offsetY = 0;
      cropper.shape = "square";
      recomputeCropperBaseScale();
      openCropperModal();
    }
  } else if (action === "cropper-cancel") reopenCropperParentModal();
  else if (action === "cropper-save") saveCropperResult();
  else if (action === "edit-real-table")
    editTableModal(Number(actionElement.dataset.table));
  else if (action === "save-edit-table")
    saveEditedTable(Number(actionElement.dataset.table));
  else if (action === "edit-real-staff")
    editStaffModal(Number(actionElement.dataset.staff));
  else if (action === "save-edit-staff")
    saveEditedStaff(Number(actionElement.dataset.staff));
  else if (action === "refresh-analytics") {
    mgrAnalyticsTried = false;
    loadManagerAnalytics();
    render();
  } else if (action === "new-reservation") reservationFormModal();
  else if (action === "edit-reservation") {
    const r = cashReservations.find(
      (candidate) => candidate.id === Number(actionElement.dataset.reservation),
    );
    if (r) reservationFormModal(r);
  } else if (action === "save-reservation")
    saveReservation(Number(actionElement.dataset.reservation) || undefined);
  else if (action === "confirm-reservation")
    reservationTransition(Number(actionElement.dataset.reservation), "confirm");
  else if (action === "cancel-reservation")
    reservationTransition(Number(actionElement.dataset.reservation), "cancel");
  else if (action === "complete-reservation")
    reservationTransition(
      Number(actionElement.dataset.reservation),
      "complete",
    );
  const cartChange = target.closest("[data-cart-change]");
  if (cartChange) {
    const index = Number(cartChange.dataset.index);
    state.cart[index].quantity = Math.max(
      1,
      state.cart[index].quantity + Number(cartChange.dataset.cartChange),
    );
    saveState();
    showModal(cartDetails());
  }
  ...
  const modalQty = target.closest("[data-modal-quantity]");
  if (modalQty) {
    modalQuantity = Math.max(
      1,
      modalQuantity + Number(modalQty.dataset.modalQuantity),
    );
    const quantity = document.getElementById("modal-quantity");
    if (quantity) quantity.textContent = `${modalQuantity}`;
  }
  const category = target.closest("[data-category]");
  if (category) {
    selectedCategory = category.dataset.category;
    safeSetStorage(CUSTOMER_CATEGORY_KEY, selectedCategory);
    render();
  }
  const customerTab = target.closest("[data-customer-view]");
  if (customerTab) {
    customerView = customerTab.dataset.customerView;
    render();
  }
  const staffTab = target.closest("[data-staff-section]");
  if (staffTab) {
    ...
    if (state.staffRole === "MANAGER" && staffSection === "overview")
      mgrAnalyticsTried = false;
    render();
  }
  const mgrMenuFilterButton = target.closest("[data-mgr-menu-filter]");
  if (mgrMenuFilterButton) {
    const value = mgrMenuFilterButton.dataset.mgrMenuFilter || "ALL";
    mgrMenuFilter = value === "ALL" ? "ALL" : Number(value);
    render();
  }
  const loginRole = target.closest("[data-login-role]");
  if (loginRole) {
    document
      .querySelectorAll("[data-login-role]")
      .forEach((button) => button.classList.remove("active"));
    loginRole.classList.add("active");
  }
  const method = target.closest("[data-payment-method]");
  if (method) {
    document
  ...
  const filter = target.closest("[data-cashier-filter]");
  if (filter) {
    cashierFilter = filter.dataset.cashierFilter || "ALL";
    render();
  }
  const reservationFilterButton = target.closest("[data-reservation-filter]");
  if (reservationFilterButton) {
    reservationFilter =
      reservationFilterButton.dataset.reservationFilter || "ALL";
    render();
  }
  const stock = target.closest("[data-stock]");
  if (stock) {
    const id = stock.dataset.stock || "";
    state.availability[id] =
      state.availability[id] === "AVAILABLE" ? "SOLD_OUT" : "AVAILABLE";
    saveState();
    render();
  }
  const tableState = target.closest("[data-table-state]");
  if (tableState) {
    const table = state.tables.find(
      (candidate) => candidate.id === tableState.dataset.tableState,
    );
    if (table) {
      table.status = tableState.dataset.nextTable;
      saveState();
      render();
      showToast(`${table.label} updated`, formatStatus(table.status), "info");
    }
  }
  ...
  const kitchenStockBtn = target.closest("[data-kitchen-sold-out]");
  if (kitchenStockBtn) {
...
async function handleLogin() {
  ...
  const selected = document.querySelector("[data-login-role].active");
  const role = selected?.dataset.loginRole || "CASHIER";
  ...
  if (
    ...
    render();
    showToast(
      `Signed in as ${formatStatus(role)}`,
      "Connected to the live PM Café database.",
      "success",
    );
    if (role === "KITCHEN") {
      loadKitchenOrders();
  ...
  render();
  showToast(
    `Signed in as ${formatStatus(state.staffRole)}`,
    "Your role-specific workspace is ready.",
    "success",
  );
}
...
document.addEventListener("input", (event) => {
  const target = event.target;
  if (target.id === "kitchen-search") {
    kitchenSearch = target.value;
    render();
    const search = document.getElementById("kitchen-search");
    search?.focus();
...
document.addEventListener("input", (event) => {
  ...
  render();
  const search = document.getElementById("mgr-menu-search");
  search?.focus();
  search?.setSelectionRange(search.value.length, search.value.length);
});
...
document.addEventListener("change", (event) => {
  const target = event.target;
  const tableStatus = target.closest("[data-mgr-table-status]");
  if (tableStatus)
    updateRealTableStatus(
      Number(tableStatus.dataset.mgrTableStatus),
      tableStatus.value,
    );
  if (target.id === "attendance-date") {
    mgrAttendanceDate = target.value;
    mgrAttendance = null;
    render();
  }
  ...
  if (fileInput.id === "item-image" && fileInput.files?.[0])
    openCropperForFile(fileInput.files[0], "item");
  if (fileInput.id === "cat-image" && fileInput.files?.[0])
    openCropperForFile(fileInput.files[0], "category");
});
...
document.addEventListener("mouseup", () => {
  if (cropper) cropper.dragging = false;
});
...
window.addEventListener("storage", (event) => {
  ...
  if (event.key === "pm-cafe-state") {
    ...
    showToast(
      "Board updated",
      "Another café station changed the live session.",
      "info",
    );
  }
});
...
async function hydrateMenu() {
  ...
  menu = records.map((record) => {
    // Safe category: handles null, undefined, or object from API
    const categoryName =
      String(
    ...
    return {
      ...
      hasIngredients: !!record.ingredients,
      price: Number(record.price),
      image:
        record.image_url || record.image
          ? mediaUrl(record.image_url || record.image)
          : "",
      availability:
        record.sold_out || !record.active ? "SOLD_OUT" : "AVAILABLE",
      tags: record.is_complete_meal ? ["Complete meal"] : [categoryName],
      options: record.options || [],
      addons: record.addons || [],
    };
  });
  categories = [...new Set(menu.map((item) => item.category))];
  // The Offers tab always sits last (after Complete Meals), even before the manager has added
  // an offer, so customers see it and an empty message instead of a missing category.
  if (!categories.some((name) => name.trim().toLowerCase() === "offers"))
    categories.push("Offers");
...
document.addEventListener(
  "scroll",
  (event) => {
    if (event.target.matches?.(".menu-section .category-row"))
      safeSetStorage(CUSTOMER_CATEGORY_SCROLL_KEY, String(event.target.scrollLeft));
  },
  true,
);
async function refreshMenu() {
  ...
  const categoryScroll = document.querySelector(".category-row")?.scrollLeft || 0;
  try {
    await hydrateMenu();
    render();
    const restoredRail = document.querySelector(".category-row");
...
function handlePaymentReturn() {
  ...
  if (payment === "success") {
    showToast(
      "Payment submitted",
      "Stripe returned you to PM Café. We are waiting for the verified server confirmation.",
      "info",
    );
  } else if (payment === "cancelled") {
    showToast(
      "Payment cancelled",
      "No payment was marked successful. You can try checkout again.",
      "warning",
    );
  }
